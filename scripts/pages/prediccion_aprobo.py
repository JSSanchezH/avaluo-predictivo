...
def load_resources():
    ...
    onehot_path = os.path.join(
        base_path, "..", "models", "prediccion_aprobo", "onehotencoder.pkl"
    )
    scaler_path = os.path.join(
        base_path, "..", "models", "prediccion_aprobo", "standard_scaler.pkl"
    )
    model_path = os.path.join(
        base_path, "..", "models", "prediccion_aprobo", "rf_model.pkl"
    )
...
examen_admision = st.number_input(
    "Examen de admisión Universidad", min_value=0.0, max_value=5.0, step=0.01
)

# --- Crear DataFrame con los datos ingresados ---
input_data = pd.DataFrame(
    [[felder, examen_admision]], columns=["Felder", "Examen_admisión_Universidad"]
)
...
try:
    ...
    felder_encoded_df = pd.DataFrame(
        felder_encoded, columns=onehot_encoder.get_feature_names_out(["Felder"])
    )
...
if st.button("Predecir"):
    try:
        ...
        st.write(
            ...
            getattr(model, "feature_names_in_", "No definidas"),
        )
